...
void print_full_time(void)
{
    ...
    for (uint8_t i=0; i<30; i+=2)
    {
        ...
        display_full_buf[i>>1][1] = tdDigital15table(h_h, i+1) | (tdDigital15table(h, i) >> 4);
        display_full_buf[i>>1][2] = (tdDigital15table(h, i) << 4) | (tdDigital15table(h, i+1) >> 4);
        display_full_buf[i>>1][3] = tdDigital15table(m_h, i) >> 1;
        display_full_buf[i>>1][4] = (tdDigital15table(m_h, i) << 7) | (tdDigital15table(m_h, i+1) >> 1) | (tdDigital15table(m, i) >> 5);
        display_full_buf[i>>1][5] = (tdDigital15table(m, i) << 3) | (tdDigital15table(m, i+1) >> 5);
    }
}
...
void print_top_time(void)
{
    ...
    for (uint8_t i=0; i<7; i++)
    {
        display_top_buf[i][1] = dDigital7table(h, i);
        display_top_buf[i][2] = dDigital7table(m_h, i) >> 2;
        display_top_buf[i][3] = dDigital7table(m, i) >> 1;
        display_top_buf[i][4] = dDigital7table(s_h, i) >> 3;
        display_top_buf[i][5] = dDigital7table(s, i) >> 2;
    }
...
void print_bottom_date(void)
{
    ...
    if(col_shift)
    {
        for (uint8_t j=0; j<7; j++)
        {
            display_bottom_buf[j][col] |= dDigital7table(date, j) >> col_shift;
        }
        if((col_shift + 5) > 7)
        {
            col++;
    ...
    else
    {
        for (uint8_t j=0; j<7; j++)
        {
            display_bottom_buf[j][col] = dDigital7table(date, j);
        }
    }
    shift += 5 + 2;
